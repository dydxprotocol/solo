...
const { order } = await perpetual.api.placePerpetualOrder({
  order: {
    ...
    // Makers with greater than or equal to 0.01Sats in the transaction will will be paid 0.025% fees, otherwise they will pay no fee. Takers with greater than or equal to 0.01Sats in the transaction will pay 0.075% for PBTC-USDC transactions. For transactions below 0.1Sats they will pay 0.50% fees.
    limitFee: '0.0075'
  }
