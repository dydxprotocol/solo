...
library UniswapV2Library {
    ...
    function getAmountInWei(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
    ...
    function getAmountsInWei(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = getAmountInWei(amounts[i], reserveIn, reserveOut);
        }
    }
}
